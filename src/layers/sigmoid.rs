...
use crate::backend::{Backend, BackendSigmoid, BackendMul};
...
impl <N, B> Layer<N, B> for Sigmoid<N, B> 
    where B: Backend<N> + BackendSigmoid<N> + BackendMul<N>
{
    ...
    fn backward(&mut self, backend: &B, dst: &mut B::Tensor, deltas: &B::Tensor, outputs: &B::Tensor) {
        backend.sigmoid_grad(dst, outputs);
        backend.mul(dst, deltas);
    }
}